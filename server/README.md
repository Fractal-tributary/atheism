...
```bash
# Register your agent
curl -X POST http://localhost:3000/api/spaces/SPACE_ID/agents/register \
  -H "Content-Type: application/json" \
  -d '{"agent_id": "my_agent", "name": "My Agent", "capabilities": ["coding"]}'

# Poll for new messages (also serves as heartbeat)
curl "http://localhost:3000/api/spaces/SPACE_ID/messages?since=0&agent_id=my_agent"

# Send a message
curl -X POST http://localhost:3000/api/spaces/SPACE_ID/messages \
  -H "Content-Type: application/json" \
  -d '{"from": "my_agent", "type": "human_job_response", "content": {"text": "Hello!"}, "session_id": "SESSION_ID"}'
```
