...
export async function abortActiveJob(sessionId: string, reason: string, agentId?: string): Promise<void> {
...
function buildInboundHistory(messages: AtheismMessage[], selfAgentId: string): Array<{ sender: string; body: string; timestamp?: number }> {
  ...
  return messages.map(msg => {
    ...
    const sender = msg.from_agent === "human" 
      ? "human" 
      : isMe ? "assistant" : (msg.from_name || msg.from_agent);
    ...
    return { sender, body, timestamp: new Date(msg.timestamp).getTime() };
...
export async function handleAtheismMessage(params: {
  ...
  try {
    ...
    const inboundHistory = buildInboundHistory(
      fullHistory.filter(m => m.message_id !== msgId),
      agentId,
    );
    
    // 🆕 如果有 summary，在 history 最前面插入
    ...
    // 🆕 如果有 ledger，在 summary 之后插入（索引体，补充叙事体的 summary）
    if (ledgerRendered) {
      // 找到 summary 之后的位置插入（如果有 summary 则在其后，否则在最前面）
      const insertIdx = sessionSummary?.summary_text ? 1 : 0;
      inboundHistory.splice(insertIdx, 0, {
        sender: "system",
        body: `[Collaboration Ledger]\n${ledgerRendered}`,
        timestamp: 0,
      });
    }
    ...
    // 协作协议使用当前 agentId
    const groupSystemPrompt = await buildGroupSystemPrompt(config, agentId, onlineAgents, spaceId, sessionId);
