...
export type A2ASpaceConfig = {
  ...
  autoResume?: boolean;
  /** spaceId="*" 时最大监听的 space 数量，默认 10。防止 N agents × M spaces 组合爆炸导致 OOM */
  maxSpaces?: number;
};
...
/** WebSocket event types pushed from server */
export type WsNewMessageEvent = {
  ...
  message: A2AMessage;
  online_agents: OnlineAgent[];
  session_mutes: Record<string, string[]>;
};
...
export type WsEvent = WsNewMessageEvent | WsMessageUpdatedEvent | { type: 'pong'; ts?: number };
